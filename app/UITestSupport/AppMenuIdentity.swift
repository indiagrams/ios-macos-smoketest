...
//
// THE EXPECTED NAME COMES FROM AN EXISTING MEASURED MECHANISM, NEVER A LITERAL. This repository
// resolves the running app's own name at runtime in exactly one place —
// `app/MacOSUITests/VisibleStringSweep.swift`'s `productName`, which `SweepDriver.swift:268` already
// leans on: the application element's own rendered text FIRST (measured EMPTY on macOS,
// `VisibleStringSweep.swift:43-48`), and only then the SECOND distinct menu-bar-item title found
// while walking one accessibility snapshot in document order (`SweepDriver.swift`'s
// `harvest(_:inherited:into:)`) — never a live `menuBarItems.element(boundBy:)` query, which is the
// mechanism this file exists to stop trusting for the CLICK. `productName` is `private` inside a
// different UI-test target's leaf test class and cannot be imported from here, so this file
// reproduces its exact rule rather than restating it differently.
//
...
// menu bar this file addresses does not exist on iOS.
    ...
    /// The app's own name, resolved with the SAME rule `productName` applies — see the file
    /// header. Not a member of `XCTestCase` or `XCUIApplication`: it takes only what it needs, so
    /// it stays testable and short on its own.
    private func resolvedApplicationName(
        applicationRenderedText: String,
        entries: [XCUIApplication.MenuBarSnapshotEntry]
    ) -> (value: String, source: String) {
        if !applicationRenderedText.isEmpty {
            return (applicationRenderedText, "application-element-rendered-text")
        }
        if entries.count > 1 {
            return (entries[1].title, "menu-bar-application-item")
        }
        return ("", "none")
    }
    ...
    extension XCTestCase {
        ...
        func selectApplicationMenuBarItemByIdentity(
            ...
            let expected = resolvedApplicationName(
                applicationRenderedText: applicationRenderedText, entries: entries
            )
            recordMenuBarEnumeration(entries, expected: expected)

            guard !expected.value.isEmpty else {
                XCTFail(
                    "the app's own name could not be resolved (source=none) — cannot select the menu bar "
                        + "item by identity. enumerated: \(entries.map(\.title).joined(separator: " | "))",
                    file: file, line: line
                )
                return app.menuBarItems.firstMatch
            }
