...
> [#34](https://github.com/indiagrams/ios-macos-smoketest/issues/34) +
> [#38](https://github.com/indiagrams/ios-macos-smoketest/issues/38).
>
...
| `info.properties.CFBundleDisplayName: SmokeApp` | `infoPlist: .extendingDefault(with: ["CFBundleDisplayName": "SmokeApp", ...])` |
| `CODE_SIGN_ENTITLEMENTS: iOS/SmokeApp.entitlements` | `entitlements: .file(path: "iOS/SmokeApp.entitlements")` |
| target-level `settings.base` | `Target.target(settings: .settings(base: [...]))` (overrides project-level) |
| `dependencies: [target: SmokeApp-iOS]` | `dependencies: [.target(name: "SmokeApp-iOS")]` |
...
> XcodeGen, `SmokeAppUITests: [test]` declares the target builds for
...
> the UI test target in both, `xcodebuild build -scheme SmokeApp-iOS`
...
```diff
-brew "xcodegen"        # app/project.yml → SmokeApp.xcodeproj
+cask "tuist"           # app/Project.swift → SmokeApp.xcodeproj
```
...
+# Tuist-generated project (regenerated from app/Project.swift)
 app/SmokeApp.xcodeproj
+app/SmokeApp.xcworkspace
+app/Derived/
+.tuist/
```
...
[issue #34](https://github.com/indiagrams/ios-macos-smoketest/issues/34))
...
shasum -a 256 \
  ...
  ~/Library/Developer/Xcode/DerivedData/SmokeApp-*/Build/Products/Debug/SmokeApp_macOS.app/Contents/Resources/AppIcon.icns
...
- **Tuist generates BOTH `SmokeApp.xcodeproj` AND `SmokeApp.xcworkspace`.**
  XcodeGen only generates the `.xcodeproj`. The existing build commands
  in `ci/local-check.sh` and `ci/local-release-check.sh` use
  `-project app/SmokeApp.xcodeproj` — they keep working. If you ever
...
- **Product name suffix.** Tuist sanitizes `SmokeApp-iOS` → `SmokeApp_iOS`
  for `PRODUCT_NAME` and the .app bundle name. XcodeGen preserves the
  hyphen. If your release pipeline assumed `SmokeApp-iOS.app`, you'll
  see `SmokeApp_iOS.app`. The template's `fastlane/Fastfile` and
  `ci/local-release-check.sh` already write the final `.ipa` / `.pkg`
  with version-pattern names (`SmokeApp-<version>.ipa`), which are
...
- 4 targets: `SmokeApp-iOS`, `SmokeApp-macOS`, `SmokeAppUITests`,
  `SmokeAppMacOSUITests`
- 2 schemes: `SmokeApp-iOS`, `SmokeApp-macOS` (UI test targets in
