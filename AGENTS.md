...
| Do **not** sed-substitute `SmokeApp` literals | The template uses env-driven resolution everywhere it matters | Run `bin/rename.sh MyApp com.example.myapp "My App" --email=you@example.com` once at fork creation; afterward, the value is set in `.bootstrap.env` |
...
| App entitlements | `app/iOS/SmokeApp.entitlements`, `app/macOS/SmokeApp.entitlements` | After fork rename, basename becomes `<APP_NAME>.entitlements` |
...
4. If the feature requires a new Apple capability (Push, iCloud, App Groups, Sign in with Apple, Wallet, CloudKit):
   ...
   - Add the entitlement to `app/iOS/SmokeApp.entitlements` and/or `app/macOS/SmokeApp.entitlements`
...
- **Unit tests**: `make check` runs an unsigned iOS device build with the test plan. From the project root: `bundle exec fastlane scan --workspace app/SmokeApp.xcworkspace --scheme SmokeApp-iOS` for a direct invocation. Don't write custom test runners — the project generator wires schemes correctly.
