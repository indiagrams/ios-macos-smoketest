...
extension PrivacyLinkTests {
    ...
    func thePrivacyItem(in menu: XCUIElement, on surface: String) throws -> XCUIElement {
        ...
        // entry's text — renderedText, falling back to title when empty, since AXTitle is where
        // AppKit publishes a menu item's text and the shared read layer does not ask for it —
        // and select the one that equals `privacyPolicyTitle`, rather than trusting a fixed
        // ordinal (adversarial #14 / C-19e).
        let entries = menu.descendants(matching: .menuItem)
        let population = entries.count
        var titles: [String] = []
        var matches: [Int] = []
