...
| macOS window-activation dance | `app/UITestSupport/TestRobot.swift`, `presentWindow(within:)`; the earlier in-tree form is `app/MacOSUITests/ScreenshotDriver.swift` | After launch, `app.activate()` only when the app is not already frontmost (D-151a), then a `File ▸ New Window` menu fallback when no window appears within 8 s. On CI runners and some local machines the app launches in the background, and every query then misses. |
