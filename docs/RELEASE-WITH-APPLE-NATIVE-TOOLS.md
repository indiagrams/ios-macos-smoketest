...
> migration plan. Tracked in [#35](https://github.com/indiagrams/ios-macos-smoketest/issues/35).
...
- Bundle ID: `com.indiagram.smokeapp` (substitute yours)
- Scheme: `SmokeApp-iOS` / `SmokeApp-macOS`
- Team ID: read from `.bootstrap.env` as `FASTLANE_TEAM_ID` (set by `make init` + your edits)
- Build artifacts: `build/SmokeApp-<version>.ipa`, `build/SmokeApp-<version>.pkg`
...
WORK_DIR="$(mktemp -d)"
IOS_ARCHIVE="$WORK_DIR/SmokeApp-iOS.xcarchive"

xcodebuild archive \
  -project app/SmokeApp.xcodeproj \
  -scheme SmokeApp-iOS \
...
IPA_DEST="build/SmokeApp-${VERSION}.ipa"
cp "$IPA_SRC" "$IPA_DEST"
...
xcrun altool --upload-package "$IPA_DEST" \
  --type ios \
  --apple-id "com.indiagram.smokeapp" \
  --bundle-version "$VERSION" \
  --bundle-short-version-string "$VERSION" \
  --bundle-id "com.indiagram.smokeapp" \
...
```bash
MACOS_ARCHIVE="$WORK_DIR/SmokeApp-macOS.xcarchive"

xcodebuild archive \
  -project app/SmokeApp.xcodeproj \
  -scheme SmokeApp-macOS \
  -configuration Release \
...
PKG_DEST="build/SmokeApp-${VERSION}.pkg"
