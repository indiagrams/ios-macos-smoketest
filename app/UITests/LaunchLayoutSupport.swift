...
// be ABSENT. iOS drives with `.tap()`; the macOS twin clicks, and navigates by sidebar constant.
...
extension LaunchLayoutTests {
    ...
    /// The screen the clauses are measured at, recorded rather than assumed, and asserted to be in
    /// portrait. The SIZE is recorded, never asserted — the file header says why.
    func assertTheMeasuredEnvironment(_ surface: String) -> CGRect {
        ...
        XCTAssertGreaterThan(frame.height, frame.width, "the device is not in portrait: \(describeRect(frame))")
        let worst = frame.width <= Self.worstCase.width && frame.height <= Self.worstCase.height
        record("criterion5_screen_\(surface)=\(frame.width)x\(frame.height) worstcase=\(worst)")
        return frame
    }
    ...
    /// Tab item 0. The bar's population is asserted to be exactly 3 before the index is taken —
    /// `ShellTests`' house rule, and the reason nothing here is queried by visible text.
    func navigateToEncode() {
        visit(0, expecting: AccessibilityIdentifiers.Encode.input, named: "Encode/decode")
    }

    /// Tab item 2, the same way.
    func navigateToTimestamps() {
        visit(2, expecting: AccessibilityIdentifiers.Timestamps.input, named: "Timestamps")
    }

    /// Tap the tab item at `index` and confirm it showed the surface carrying `identifier`.
    func visit(_ index: Int, expecting identifier: String, named name: String) {
