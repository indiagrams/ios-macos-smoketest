...
gate_project_yml_in_history() {
  ...
  if [ -z "$PROJECT_YML_SHA" ]; then
   ...
   Restoring manually requires fetching project.yml from indiagrams/ios-macos-smoketest/main:
     curl -fsSL https://raw.githubusercontent.com/indiagrams/ios-macos-smoketest/main/app/project.yml > app/project.yml"
  fi
...
mutate_restore_project_yml() {
  ...
  # --diff-filter=AM). The restored content carries SmokeApp identity +
  # TEAM_ID_PLACEHOLDER + com.indiagram.smokeapp, which then breaks every
  # xcodebuild invocation (No signing certificate "...TEAM_ID_PLACEHOLDER",
  # CODE_SIGN_ENTITLEMENTS references non-existent SmokeApp.entitlements,
...
mutate_makefile() {
  ...
  sed -i '' 's|Regenerate SmokeApp.xcodeproj from app/Project.swift|Regenerate SmokeApp.xcodeproj from app/project.yml|g' Makefile
