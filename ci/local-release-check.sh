...
# Reads app/Identity.xcconfig for BUNDLE_ID (RELEASE_BUNDLE_ID env overrides).
#
...
# emptiness guard beside that grep can never fire. Last assignment wins and a
# `//` comment is not a value, as Xcode reads the file. Overridable via
...
if [ -n "${RELEASE_BUNDLE_ID:-}" ]; then
  ...
  BUNDLE_ID="$(sed -nE 's#^[[:space:]]*BUNDLE_ID[[:space:]]*=[[:space:]]*([^[:space:]/]+).*#\1#p' app/Identity.xcconfig | tail -1)"
fi
case "$BUNDLE_ID" in
  ''|*'$('*)
