...
| `appInfo` `appStoreAgeRating` | `FOUR_PLUS` — **Apple's own computation**, not a value this project set. It was `null` until plan 08-17 delivered the 28-answer declaration in `fastlane/age_rating.json`; Apple derived the band from those answers. 08-15 recorded 4+ as a *prediction* and said so; this row is the measurement, and it agrees. Alongside it Apple computed `brazilAgeRating` `L` and `brazilAgeRatingV2` `SELF_RATED_L`; `australiaAgeRating`, `franceAgeRating` and `koreaAgeRating` are still `null`, which is Apple's behaviour and not an omission here | 2026-09-11 | `appInfo` `2b0a2b50-1b4a-46d3-b888-03be64184aad` on record `6807393045` | per phase | `/opt/homebrew/opt/ruby@3.3/bin/bundle exec ruby -e 'require "spaceship"; require "base64"; Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(key_id: ENV["ASC_API_KEY_ID"], issuer_id: ENV["ASC_API_KEY_ISSUER_ID"], key: Base64.decode64(ENV["ASC_API_KEY_P8_BASE64"])); Spaceship::ConnectAPI.get_app_infos(app_id: "6807393045", includes: "primaryCategory,secondaryCategory").to_models.each { \| i\| puts "#{i.id} age=#{i.app_store_age_rating} primary=#{i.primary_category&.id.inspect} secondary=#{i.secondary_category&.id.inspect}" }'` |
| `appInfo` primary / secondary category | `DEVELOPER_TOOLS` primary, secondary `null`, and all four subcategory slots `null` — D-123, and `DEVELOPER_TOOLS` has no subcategories to set. Both were `null` until plan 08-17. The value is the **API identifier**, never the display name: `deliver`'s `map_category_from_itc` returns an unrecognised string unchanged, so `Developer Tools` would ship an id that does not exist with no local error. Agrees with the shipped `LSApplicationCategoryType` `public.app-category.developer-tools` at all five sites (`app/project.yml` ×2, `app/Project.swift` ×2, `app/macOS/Generated-Info.plist`) | 2026-09-11 | `appInfo` `2b0a2b50-1b4a-46d3-b888-03be64184aad` on record `6807393045` | per phase | `/opt/homebrew/opt/ruby@3.3/bin/bundle exec ruby -e 'require "spaceship"; require "base64"; Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(key_id: ENV["ASC_API_KEY_ID"], issuer_id: ENV["ASC_API_KEY_ISSUER_ID"], key: Base64.decode64(ENV["ASC_API_KEY_P8_BASE64"])); Spaceship::ConnectAPI.get_app_infos(app_id: "6807393045", includes: "primaryCategory,secondaryCategory").to_models.each { \| i\| puts "#{i.id} age=#{i.app_store_age_rating} primary=#{i.primary_category&.id.inspect} secondary=#{i.secondary_category&.id.inspect}" }'` |
| `appStoreVersion` `releaseType`, both platforms | `MANUAL` on IOS and on MAC_OS, changed from `AFTER_APPROVAL` by plan 08-17. `deliver` maps `automatic_release: false` to this and sends it on every metadata upload; there is no option to leave it alone. **Read this before submitting**: an approved version will NOT release by itself — someone has to release it | 2026-09-11 | versions `e026e36c-9166-4a8c-b558-640a893f1852` (IOS) and `316a7725-4f8c-45fe-a6cb-c882ecc2966c` (MAC_OS) on record `6807393045` | per submission | `/opt/homebrew/opt/ruby@3.3/bin/bundle exec ruby -e 'require "spaceship"; require "base64"; Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(key_id: ENV["ASC_API_KEY_ID"], issuer_id: ENV["ASC_API_KEY_ISSUER_ID"], key: Base64.decode64(ENV["ASC_API_KEY_P8_BASE64"])); Spaceship::ConnectAPI.get_app_store_versions(app_id: "6807393045").to_models.each { \| v\| puts "#{v.platform} #{v.version_string} releaseType=#{v.release_type}" }'` |
| `appScreenshotSets`, both platforms | **3 sets, 8 screenshots each, 24 assets, 24 distinct** — `IOS` carries `APP_IPHONE_67` (8 x 1320x2868) and `APP_IPAD_PRO_3GEN_129` (8 x 2064x2752); `MAC_OS` carries `APP_DESKTOP` (8 x 2880x1800) and nothing else. All were `null`/0 until plan 08-18 — no screenshot had ever been uploaded to this record. Every asset `state=COMPLETE` with `errors=[]`, and every `sourceFileChecksum` maps to exactly one file on disk with the same name. The ordinal-`01` chain shot reads back FIRST in all three sets (`deliver` reorders by natural sort of the file name after upload, so the ordering a reviewer sees is decided by the file name — D-125). **The macOS display type is the proof that plan 08-10's `fastlane/Fastfile.local` override reads `fastlane/Mac_screenshots` and not the iOS directory**: iOS-sized files could not produce `APP_DESKTOP`. **Read `UL-076` before re-running either upload lane** — `deliver` left 30 assets where 24 were uploaded and exited 0 saying it had succeeded; the six duplicates were removed by a delete pass and the correct count here is the READ-BACK, never the lane's exit code | 2026-09-11 | versions `e026e36c-9166-4a8c-b558-640a893f1852` (IOS) and `316a7725-4f8c-45fe-a6cb-c882ecc2966c` (MAC_OS) on record `6807393045` | per submission | `/opt/homebrew/opt/ruby@3.3/bin/bundle exec ruby -e 'require "spaceship"; require "base64"; Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(key_id: ENV["ASC_API_KEY_ID"], issuer_id: ENV["ASC_API_KEY_ISSUER_ID"], key: Base64.decode64(ENV["ASC_API_KEY_P8_BASE64"])); Spaceship::ConnectAPI.get_app_store_versions(app_id: "6807393045").to_models.each { \| v\| v.get_app_store_version_localizations.each { \| l\| l.get_app_screenshot_sets.each { \| s\| shots = s.app_screenshots; puts "#{v.platform} #{s.screenshot_display_type} n=#{shots.length} distinct=#{shots.map(&:source_file_checksum).uniq.length} first=#{shots.first&.file_name}" } } }'` |
| Screenshot sizes this record accepts | **1320x2868, 2064x2752 and 2880x1800 were all accepted on a record that had never held a screenshot** — three sets created from zero, `errors=[]` on every asset, nothing refused at any size on either platform. This answers `08-RESEARCH.md` Open Question 2 in the NEGATIVE: the `fastlane/Snapfile` comment claiming accepted slots are "baked into the App Store record at App creation time and are sticky" has no source and gains no support from the first upload this record ever received. The question was unanswerable before an upload existed, which is why it stayed open through five plans | 2026-09-11 | localizations `0176302d-67e3-4511-a0a9-aca49068958b` (IOS) and `28d07571-7251-4572-9929-d6886173ef61` (MAC_OS) on record `6807393045` | per phase | `/opt/homebrew/opt/ruby@3.3/bin/bundle exec ruby -e 'require "spaceship"; require "base64"; Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(key_id: ENV["ASC_API_KEY_ID"], issuer_id: ENV["ASC_API_KEY_ISSUER_ID"], key: Base64.decode64(ENV["ASC_API_KEY_P8_BASE64"])); Spaceship::ConnectAPI.get_app_store_versions(app_id: "6807393045").to_models.each { \| v\| v.get_app_store_version_localizations.each { \| l\| l.get_app_screenshot_sets.each { \| s\| shots = s.app_screenshots; puts "#{v.platform} #{s.screenshot_display_type} n=#{shots.length} distinct=#{shots.map(&:source_file_checksum).uniq.length} first=#{shots.first&.file_name}" } } }'` |
